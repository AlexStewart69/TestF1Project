      ...
      {query.data.map((session) => (
        <li key={session.session_key}>
          <div className="container grid grid-cols-6 gap-4">
            <div className="mb-6 rounded border-2 border-slate-100 bg-slate-50 p-4 shadow-md font-mono ... text-green-600 text-2xl dark:text-sky-400">
              <p>Meeting Key:</p> {session.meeting_key}
            </div>

            <div className="mb-6 rounded border-2 border-slate-100 bg-slate-50 p-4 shadow-md font-sans ... text-green-600 text-2xl dark:text-sky-400">
              <p>Circuit Name:</p> {session.circuit_short_name}
            </div>
            <div className="mb-6 rounded border-2 border-slate-100 bg-slate-50 p-4 shadow-md font-sans ... text-yellow-600 text-2xl dark:text-sky-700">
              <p>Country:</p> {session.country_name}
            </div>
            <div className="mb-6 rounded border-2 border-slate-100 bg-slate-50 p-4 shadow-md font-mono ... text-blue-600 text-2xl dark:text-sky-700">
              <p>Date and Time:</p> {session.date_start}
            </div>
            <div className="mb-6 rounded border-2 border-slate-100 bg-slate-50 p-4 shadow-md font-sans ... text-orange-600 text-2xl dark:text-sky-800">
              <p>Type of Meeting:</p> {session.session_name}
            </div>
            <div className="mb-6 rounded border-2 border-slate-100 bg-slate-50 p-4 shadow-xl/10 ... font-mono ... text-green-600 text-2xl dark:text-sky-400">
              <Link to={`/session_result/${session.session_key}`}>
                <p>Session ID:</p> {session.session_key}
              </Link>
            </div>
          </div>
        </li>
